...
import 'package:FAB/presentation/screens/transaction/retiro/retiro_realizado_screen.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;
import 'package:flutter_dotenv/flutter_dotenv.dart';
import 'package:FAB/utils/token_service.dart'; // Importa tu servicio de tokens aquí

class RetitoScreen extends StatefulWidget {
  ...
  @override
  _RetitoScreenState createState() => _RetitoScreenState();
}

class _RetitoScreenState extends State<RetitoScreen> {
  ...
  Future<void> realizarRetiro(String userId) async {
    try {
      String? token = await SessionManager().getToken();
      final response = await http.post(
        Uri.parse('$apiUrl/withdraw/${widget.userId}'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'Authorization': 'Bearer $token',
        },
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => RetiroRealizadoScreen(
        ...
        print('Error al realizar el retiro: ${response.statusCode}');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error al realizar el retiro: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
      ...
      builder: (BuildContext context) {
        return Scaffold(
          body: AlertDialog(
            ...
            content: Text('¿Estás seguro de que deseas realizar este retiro?'),
            actions: <Widget>[
              ElevatedButton(
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  if (isSuperUser && selectedUser != null) {
                    await realizarRetiro(selectedUser!);
                  } else if (!isSuperUser) {
                    await realizarRetiro(widget.userId);
                  } else {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
